...
if (isset($_GET['mark_read'])) {
    $id = $_GET['mark_read'];
    if ($id === 'all') {
        $stmt = $conn->prepare("UPDATE notifications SET is_read=1 WHERE is_read=0 AND (target_role='all' OR target_role='student' OR target_user_id=?)");
        $stmt->execute([$uid]);
        set_flash('success','All marked as read.');
    } else {
        $conn->prepare("UPDATE notifications SET is_read=1 WHERE id=? AND (target_role='all' OR target_role='student' OR target_user_id=?)")->execute([$id,$uid]);
...
$notifications = $conn->prepare("SELECT n.*, u.name AS creator_name FROM notifications n LEFT JOIN users u ON n.created_by=u.id WHERE n.target_role='all' OR n.target_role='student' OR n.target_user_id=? ORDER BY n.created_at DESC LIMIT 30");
$notifications->execute([$uid]); $notifications = $notifications->fetchAll();
$unread_stmt = $conn->prepare("SELECT COUNT(*) FROM notifications WHERE is_read=0 AND (target_role='all' OR target_role='student' OR target_user_id=?)");
$unread_stmt->execute([$uid]); $unread = (int)$unread_stmt->fetchColumn();
